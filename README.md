...
4. `kernels/optimized_pope.py`: keeps the same PoPE math, but avoids materializing the full expanded tensors by computing the PoPE coordinates inside the Triton kernel to minimize memory use.
...
| optimized_pope | 7.399 | 25.525 | 98.542 | 390.511 | 49.434 | 98.293 |
...
| optimized_pope | 0.63 | 1.25 | 2.50 | 5.00 | 2.25 | 4.25 |

## Why `optimized_pope` is much slower but uses much less memory
